...
def render_soil_profile(id_proyecto, cfg, cols_vwc, cols_temp, cols_pt, cols_dpt,
    ...
    svg_render_w = 700
    svg_render_h = round(700 * svg_h / 560)
    iframe_h     = 55 + svg_render_h + 40
...
.main-grid{{display:grid;grid-template-columns:160px 1fr 185px;min-height:{svg_render_h}px}}
...
.profile-area{{padding:10px;display:flex;flex-direction:column;gap:6px;align-items:center}}
.profile-wrap{{position:relative;border-radius:10px;border:1px solid #21262d;overflow:hidden;display:inline-block;margin:0 auto}}
...
<div class="main-grid">
  ...
  <div class="profile-area">
    <div class="profile-wrap" id="profile-wrap">
      <svg id="profile-svg" class="profile-svg" viewBox="0 0 560 {svg_h}" width="{svg_render_w}" height="{svg_render_h}" xmlns="http://www.w3.org/2000/svg"></svg>
    </div>
...
function buildSVG() {{
  ...
  h += `<rect x="225" y="20" width="110" height="54" rx="8" fill="#ddeeff" stroke="#80aacc" stroke-width="0.9"/>
        <rect x="238" y="12" width="84" height="12" rx="4" fill="#4a90d9" stroke="#2a70b9" stroke-width="0.6"/>
        <text x="280" y="46" text-anchor="middle" font-family="sans-serif" font-size="14" font-weight="700" fill="#1a3a5c">VMS</text>
        <text x="280" y="62" text-anchor="middle" font-family="sans-serif" font-size="10" fill="#3a6080">Estación</text>
        <line x1="323" y1="12" x2="333" y2="4" stroke="#4a7a9a" stroke-width="1.2"/>
        <circle cx="335" cy="3" r="2.5" fill="none" stroke="#4a9ad9" stroke-width="1.2"/>`;
  ...
  h += `<line x1="460" y1="${{SY}}" x2="460" y2="${{H-10}}" stroke="white" stroke-width="0.3" opacity="0.2"/>`;
  SENSORS.forEach((s, i) => {{
    const p = sensorPos(i);
    h += `<line x1="455" y1="${{p.y.toFixed(1)}}" x2="465" y2="${{p.y.toFixed(1)}}" stroke="#7dc3ff" stroke-width="0.8" opacity="0.6"/>`;
    h += `<text x="470" y="${{(p.y+4).toFixed(1)}}" font-family="sans-serif" font-size="13" fill="rgba(125,195,255,0.9)">${{s.depth}}</text>`;
  }});
  SENSORS.forEach((s, i) => {{
    const p = sensorPos(i);
...
def construir_interfaz_proyecto(id_proyecto: str):
    ...
    svg_render_w = 700
    svg_render_h = round(700 * svg_h_calc / 560)
    iframe_h     = 55 + svg_render_h + 40
